...
SELECT animals.name AS animal_name,COUNT(animals.name) AS number_visits from animals JOIN visits ON animals.id=visits.animals_id GROUP BY animals.name ORDER BY number_visits DESC LIMIT 1;
